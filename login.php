...
$already = function_exists('is_logged_in')
  ? is_logged_in()
  : (!empty($_SESSION['loggedin']));
if ($already) { header("Location: dashboard.php"); exit; }

$logo_url = 'assets/img/logo.jpg';
if (!file_exists(__DIR__ . "/$logo_url")) $logo_url = 'assets/img/logo.svg';
    ...
    if ($stmt) {
      ...
      if ($res && $res->num_rows === 1) {
        ...
        if (verify_any_password($password, $row['password'])) {
          // IMPORTANT: your app expects 'loggedin' === true
          if (session_status() !== PHP_SESSION_ACTIVE) session_start();
          session_regenerate_id(true);

          $_SESSION['loggedin']     = true;                 // <— key your app checks
          $_SESSION['id']           = (int)$row['id'];
          $_SESSION['user_id']      = (int)$row['id'];
          $_SESSION['full_name']    = $row['full_name'];
          $_SESSION['username']     = $row['username'];
          $_SESSION['role']         = $row['role'];         // 'superadmin' / 'admin' etc.
          $_SESSION['branch_id']    = (int)$row['branch_id'];

          // extra flags some pages might use
...
<body class="login-page">
  <div class="login-container">
    <div class="login-box">
      ...
      <h2 class="login-title">Purity Lab Login</h2>
